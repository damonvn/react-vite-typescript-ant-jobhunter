import { callFetchJob } from '@/configs/api';
import { convertSlug, getLocationName } from '@/configs/utils';
...
const JobCard = (props: IProps) => {
    ...
    const fetchJob = async () => {
        ...
        const res = (await callFetchJob(query)).data;
        if (res && res.data) {
                                    ...
                                    >
                                        <div className={styles["card-job-content"]}>
                                            ...
                                            <div className={styles["card-job-right"]}>
                                                ...
                                                <div className={styles["job-location"]}><EnvironmentOutlined style={{ color: '#58aaab' }} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined} />&nbsp;{getLocationName(item.location)}</div>
                                                <div><ThunderboltOutlined style={{ color: 'orange' }} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined} />&nbsp;{(item.salary + "")?.replace(/\B(?=(\d{3})+(?!\d))/g, ',')} đ</div>
