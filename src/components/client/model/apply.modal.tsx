...
import { callCreateResume, callUploadSingleFile } from "@/configs/api";
...
const ApplyModal = (props: IProps) => {
    ...
    const handleOkButton = async () => {
        ...
        if (!isAuthenticated) {
            ...
            //todo
            if (jobDetail) {
                const res = (await callCreateResume(urlCV, jobDetail?.id, user.email, user.id)).data;
                if (res.data) {
    ...
    const propsUpload: UploadProps = {
        ...
        async customRequest({ file, onSuccess, onError }: any) {
            const res = (await callUploadSingleFile(file, "resume")).data;
            if (res && res.data) {
                            ...
                            >
                                <Row gutter={[10, 10]}>
                                    ...
                                    <Col span={24}>
                                        ...
                                        >
                                            <Upload {...propsUpload}>
                                                <Button icon={<UploadOutlined onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined} />}>Tải lên CV của bạn ( Hỗ trợ *.doc, *.docx, *.pdf, and &lt; 5MB )</Button>
                                            </Upload>
                                        </ProForm.Item>
                                    </Col>
                                </Row>
