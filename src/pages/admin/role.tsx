...
import { callDeleteRole, callFetchPermission } from "@/configs/api";
import queryString from 'query-string';
import { fetchRole } from "@/redux/slices/roleSlide";
...
const RolePage = () => {
    ...
    useEffect(() => {
        const init = async () => {
            const res = (await callFetchPermission(`page=1&size=100`)).data;
            if (res.data?.result) {
    ...
    const handleDeleteRole = async (id: string | undefined) => {
        if (id) {
            const res = (await callDeleteRole(id)).data;
